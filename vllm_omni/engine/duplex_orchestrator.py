...
class DuplexOrchestrator(Orchestrator, DuplexStagePort):
    ...
    async def _route_output(
        ...
        await self._maybe_forward_aura_sentence_tts(stage_id, replica_id, output, req_state)
        await super()._route_output(stage_id, replica_id, output, req_state, stage_metrics)

    async def _maybe_forward_aura_sentence_tts(
        self,
        stage_id: int,
        replica_id: int,
        output: Any,
        ...
        edge that clears ``additional_information.text``. Talker stays
        resumable until Stage1 finishes.
        """
        if not req_state.session_owned or stage_id != 1:
            return
        if stage_id + 1 > req_state.final_stage_id:
            return
        next_client = self.stage_pools[stage_id + 1].stage_client
        ...
        raw_text = self._stage1_tts_text(output)
        ...
        if prompt is not None:
            ...
            if finished and not bridge.get("history_committed"):
                info["aura_tts_partial"] = False
                commit_duplex_stage1_history(info, _strip_assistant_text(raw_text) or raw_text)
                info["aura_tts_partial"] = True
                bridge["history_committed"] = True

        view = _SentenceTtsOutput(str(getattr(output, "request_id", req_state.request_id)), chunk)
        logger.info(
            "[DuplexOrchestrator] sentence TTS req=%s finished=%s close_only=%s text_len=%d",
    ...
    def _stage1_tts_text(self, output: Any) -> str:
        ...
        if isinstance(token_ids, list) and token_ids:
            processor = self.stage_pools[1].output_processor
