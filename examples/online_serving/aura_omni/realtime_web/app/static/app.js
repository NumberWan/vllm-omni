(() => {
  ...
  // AURA PTT client. UI shell adapted from minicpmo/realtime_web, but this file
  // lives under aura_omni/ and does not change the MiniCPM demo.
  const config = window.AURA_REALTIME_CONFIG || {};
  ...
  // ~10 ms zeros @ 16 kHz for vision-only commits (is_speech=false).
  const SILENT_PCM_SAMPLES = 160;
  const DEFAULT_PROMPT = '你是 AURA，一個有幫助的語音助手。請用簡短清楚的中文回答使用者的問題。';

  if (systemPromptInput) {
    systemPromptInput.value = config.instructions || DEFAULT_PROMPT;
  }
  ...
  function sendAppend(pcm, { isSpeech, frames } = {}) {
    ...
    const appendEvent = {
      ...
      is_speech: Boolean(isSpeech),
    };
    if (frames && frames.length) appendEvent.video_frames = frames;
  ...
  function flushCapture() {
    ...
    if (!pttHeld || pendingCapture.length === 0) {
      if (!pttHeld) pendingCapture = [];
      return;
    }
  ...
  function handleEvent(event) {
    ...
    switch (event.type) {
      ...
      case 'response.text.delta':
        addTranscript('assistant', event.delta || event.text || '');
        break;
      ...
      case 'response.text.done':
        finishTranscript('assistant', event.text || event.transcript || '');
        break;
      ...
      case 'error':
        ...
        runtimeDetail.textContent = String(
          (event.error && event.error.message) || event.error || event.code || 'Server error',
        );
        break;
      default:
        break;
  ...
  async function openCapture() {
    ...
    captureNode.port.onmessage = (message) => {
      ...
      updateMeter(pcm);
      if (running && pttHeld) pendingCapture.push(pcm);
    };
  ...
  function openSocket() {
    return new Promise((resolve, reject) => {
      ...
      socket.onopen = () => {
        ...
        runtimeDetail.textContent = `${captureRate} Hz capture / ${playbackRate} Hz playback · AURA PTT`;
  ...
  async function startSession() {
    ...
    try {
      ...
      setConnection('Connected', 'online');
      setModel('Ready (PTT)');
      appendLog('session started · AURA PTT');
    } catch (error) {
  ...
  async function startCamera() {
    ...
    appendLog('camera on (frames ride PTT; vision-follow uses is_speech=false)');
  }
