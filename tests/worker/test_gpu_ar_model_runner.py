...
def _make_runner(
    ...
    downstream_req_ids: set[str],
    *,
    accumulate_full_payload: bool = False,
) -> GPUARModelRunner:
    ...
    runner._should_accumulate_full_payload_output = lambda: accumulate_full_payload
    return runner
...
def test_resolve_pooler_payload_req_ids_text_async_chunk_skips_even_with_downstream():
    """AURA stage1: engine_output_type=text, async_chunk, still has TTS downstream.

    Wire multimodal is discarded; skipping pooler avoids per-step hidden D2H.
    """
    runner = _make_runner(
        engine_output_type="text",
        downstream_req_ids={"r1", "r2"},
        accumulate_full_payload=False,
    )
...
def test_resolve_pooler_payload_req_ids_text_accumulate_keeps_downstream():
    runner = _make_runner(
        engine_output_type="text",
        downstream_req_ids={"r2"},
        accumulate_full_payload=True,
    )
